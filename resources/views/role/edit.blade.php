        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('role.update', $role) }}" class="space-y-4" method="post">
                        @csrf
                        @method('put')
                        <div class="flex flex-col space-y-2">
                            <x-input-label for="name" :value="'Nama'" />
                            <x-text-input type="text" id="name" name="name" value="{{ $role->name }}" />
                        </div>
                        ...
                        <div class="flex flex-col space-y-2">
                            ...
                            <x-select class="select2" id="permissions" name="permissions[]" multiple>
                                ...
                                <option value="{{ $permission->name }}" {{ $role->permissions->contains('id', $permission->id) ? 'selected': '' }}>{{ $permission->name }}</option>
                                @endforeach
                            </x-select>
                        </div>
